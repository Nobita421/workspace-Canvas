    ...
    return (
        <motion.div
            className={`absolute rounded-xl border-2 transition-shadow group flex flex-col
                ...
                touch-none
            `}
            animate={{ x: data.x, y: data.y, width: data.width || 300, height: data.height || 300 }}
            ...
            <div className="p-3 border-b border-inherit/20 flex justify-between items-start">
                ...
                <div className="flex gap-1 zone-btn">
                    <button
                        ...
                        className={`p-1 rounded hover:bg-black/10 transition-colors ${data.locked ? 'text-indigo-500' : 'text-slate-400 opacity-0 group-hover:opacity-100'}`}
                        aria-label={data.locked ? 'Unlock zone' : 'Lock zone'}
                    ...
                    {!data.locked && (
                        <button
                            onClick={(e) => { e.stopPropagation(); const idx = ZONE_COLORS.findIndex(c => c.id === (data.color || 'slate')); const nextColor = ZONE_COLORS[(idx + 1) % ZONE_COLORS.length].id; updateThread(data.id, { color: nextColor }); }}
                            className={`w-3 h-3 mt-1 rounded-full opacity-50 hover:opacity-100 transition-opacity ${darkMode ? 'bg-slate-600' : 'bg-slate-400'}`}
                            aria-label="Cycle zone color"
                            title="Change color"
                        />
                    )}
                </div>
